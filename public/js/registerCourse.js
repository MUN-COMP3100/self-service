...
coursesContainer.addEventListener("click", async (e) => {
  const isRegisterButton =
    e.target.nodeName === "BUTTON" && e.target.classList.contains("registerBtn")
  const isViewScheduleButton =
    e.target.nodeName === "BUTTON" && e.target.classList.contains("scheduleBtn")
  const isFavoriteButton =
    e.target.nodeName === "BUTTON" && e.target.classList.contains("favoriteBtn")
  ...
  if (isRegisterButton) {
    ...
    try {
      ...
      if (found)
        return showMessage(false, "You are already registered for this course")
      ...
      if (error.response?.status === 409) {
        showMessage(
          ...
          error.response.data.messages[0] ||
            "This course has a time conflict with another course"
        )
      } else {
    ...
    modal.addEventListener("click", (eModal) => {
      const isCloseBtn =
        eModal.target.nodeName === "BUTTON" &&
        eModal.target.classList.contains("closeBtn")
